...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	...
	case participation.Table:
		query := c.Participation.Query().
			Where(participation.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, participationImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	case participation.Table:
		query := c.Participation.Query().
			Where(participation.IDIn(ids...))
		query, err := query.CollectFields(ctx, participationImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
