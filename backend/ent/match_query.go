...
func (mq *MatchQuery) QueryCreator() *UserQuery {
...
// QueryMatchParticipation chains the current query on the "match_participation" edge.
func (mq *MatchQuery) QueryMatchParticipation() *ParticipationQuery {
...
func (mq *MatchQuery) Clone() *MatchQuery {
	...
	return &MatchQuery{
		...
		withMatchParticipation: mq.withMatchParticipation.Clone(),
...
func (mq *MatchQuery) WithCreator(opts ...func(*UserQuery)) *MatchQuery {
	query := (&UserClient{config: mq.config}).Query()
...
// WithMatchParticipation tells the query-builder to eager-load the nodes that are connected to
// the "match_participation" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MatchQuery) WithMatchParticipation(opts ...func(*ParticipationQuery)) *MatchQuery {
	query := (&ParticipationClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withMatchParticipation = query
	return mq
}
...
func (mq *MatchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Match, error) {
	...
	if query := mq.withMatchParticipation; query != nil {
...
func (mq *MatchQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Match, init func(*Match), assign func(*Match, *User)) error {
...
func (mq *MatchQuery) loadMatchParticipation(ctx context.Context, query *ParticipationQuery, nodes []*Match, init func(*Match), assign func(*Match, *Participation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(participation.FieldMatchID)
	}
	query.Where(predicate.Participation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(match.MatchParticipationColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
