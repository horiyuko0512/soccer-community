...
func (uq *UserQuery) QueryMatches() *MatchQuery {
...
// QueryUserParticipation chains the current query on the "user_participation" edge.
func (uq *UserQuery) QueryUserParticipation() *ParticipationQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserParticipation: uq.withUserParticipation.Clone(),
...
func (uq *UserQuery) WithMatches(opts ...func(*MatchQuery)) *UserQuery {
	query := (&MatchClient{config: uq.config}).Query()
...
// WithUserParticipation tells the query-builder to eager-load the nodes that are connected to
// the "user_participation" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserParticipation(opts ...func(*ParticipationQuery)) *UserQuery {
	query := (&ParticipationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserParticipation = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMatches; query != nil {
		if err := uq.loadMatches(ctx, query, nodes,
			func(n *User) { n.Edges.Matches = []*Match{} },
			func(n *User, e *Match) { n.Edges.Matches = append(n.Edges.Matches, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withUserParticipation; query != nil {
...
func (uq *UserQuery) loadMatches(ctx context.Context, query *MatchQuery, nodes []*User, init func(*User), assign func(*User, *Match)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(match.FieldCreatorID)
	}
	query.Where(predicate.Match(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MatchesColumn), fks...))
	}))
...
func (uq *UserQuery) loadUserParticipation(ctx context.Context, query *ParticipationQuery, nodes []*User, init func(*User), assign func(*User, *Participation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(participation.FieldUserID)
	}
	query.Where(predicate.Participation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserParticipationColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
